...
frequent_itemsets, rules = aprioris.aprioris(df_filtered, min_support, min_confidence, e)
end_time = time.time()
...
instance1=[]
start_time = time.time()
for id_instance,instance_dict in enumerate(binarized_validation[:5]):
    ...
    reason = dt_explainer.majoritary_reason(n_iterations=10)
...
for id_instance,instance_dict in enumerate(binarized_validation[:5]):
    ...
    dt_explainer.set_instance(raw_validation[id_instance])
    #print("instance2",raw_validation[id_instance])
    reason1 = dt_explainer.majoritary_reason(n_iterations=10)
...
moreasen2=len_reason_theorie2/5
end_time = time.time()
...
plt.savefig(name+"_comparaison_tailles_rules.png")  # Sauvegarder sous forme d'image
...
# Sauvegarde du graphique
plt.savefig(name+"_comparaison_nombre_changement.png")  # Sauvegarder sous forme d'image
plt.show()  # Afficher le graphique
